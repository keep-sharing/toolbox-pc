...
class QunpUCXX14 : public QObject
{
    Q_OBJECT
...
public:
    ...
    */
    ...
    explicit QunpUCXX14(QObject *parent = nullptr);
    ...
    unp_state_t pn532_init(void);
    unp_state_t pn532_check(void);
    unp_state_t pn532_deinit(void);
    void flash_addr_init(uint8_t * addr);
    void status_addr_init(uint8_t * addr);

    //read
    unp_state_t nfc_get_status(status_msg_t *status_buf);
    unp_state_t nfc_get_flash(flash_user_params_t * flash_buf);
    unp_state_t nfc_get_device(flash_user_params_t * flash_buf);
    unp_state_t nfc_get_fetch(uint8_t *fetch_buf, uint8_t type, uint8_t channel);
    unp_state_t nfc_get_password(uint8_t * password);
    unp_state_t nfc_get_gpio_counter(uint8_t gpionum, uint8_t *value);
    //write
    unp_state_t nfc_set_flash(flash_user_params_t *flash, uint32_t len, uint8_t *password, uint32_t pwd_len, uint16_t flash_offset1, uint16_t flash_offset2);
    unp_state_t nfc_set_power_on(uint8_t *password, uint32_t pwd_len);
    unp_state_t nfc_set_power_off(uint8_t *password, uint32_t pwd_len);
    unp_state_t nfc_set_upgrade_request(uint8_t *addr, uint32_t len, uint8_t *password, uint32_t pwd_len, int imageVer, int imageVerLen);
    unp_state_t nfc_set_upgrade_query(void);
    unp_state_t nfc_set_op_complete (void);
    unp_state_t nfc_set_password(uint8_t *old_password,uint8_t* new_password);
    unp_state_t nfc_set_appkey(uint8_t* new_appkey, uint8_t *password, uint32_t pwd_len);
    unp_state_t nfc_set_device_restart(uint8_t * password,uint32_t pwd_len);
    unp_state_t nfc_set_device_reset(uint8_t * password,uint32_t pwd_len);
    unp_state_t nfc_set_device_reload(uint8_t * password,uint32_t pwd_len);
    unp_state_t nfc_set_device_time(uint8_t * password, uint32_t pwd_len, uint32_t timestamp, int16_t timezone);
    unp_state_t nfc_set_calibration_mode(uint8_t * password, uint32_t pwd_len, uint8_t cal);
    unp_state_t nfc_get_collect_request(uint8_t * password,uint32_t pwd_len, uint8_t type, uint8_t channel);
    unp_state_t nfc_set_gpio_status(uint8_t * password,uint32_t pwd_len, uint8_t channel, uint8_t value);
    unp_state_t nfc_set_gpio_counter_status(uint8_t * password,uint32_t pwd_len, uint8_t gpionum, uint8_t value);
    unp_state_t nfc_set_solenoid_status(uint8_t * password,uint32_t pwd_len, uint8_t num, uint8_t value);
...
private:
...
#ifdef SUPPORT_UPGRADE_UBOOT
    void deal_upgrade_uboot(uint8_t *data, uint16_t datasize);
    void deal_upgrade_uboot_ack(uint8_t *data, uint16_t datasize);
#endif
    ...
    //NFC
    uint8_t pn532_wakeup ;
    uint8_t device_is_on ;
    uint8_t recv_data[NFC_MAX_LEN+1];
    uint8_t recv_datalen ;
    uint8_t send_enable ;
    uint8_t write_enable;
    uint8_t read_enable;
    bool nfc_upgreade_flag;

    void deal_with_PN532(uint8_t *data,uint8_t data_len);
    int32_t deal_with_nfc_ack(uint8_t* data);
    void nfc_uart_send_data(uint8_t *data, uint16_t pktsize);
    void nfc_uart_recv_data(uint8_t *data, uint16_t *pktsize);

    uint8_t get_pn532_crc(uint8_t *data ,uint16_t len);
    unp_state_t send_write_param_request_nfc(uint8_t *data,uint8_t* addr);
    unp_state_t send_read_param_request_nfc(uint8_t *data, uint8_t *addr, uint16_t *pktsize=nullptr);
    unp_state_t wait_for_nfc_ack(uint8_t *data,uint16_t *pktsize);
    unp_state_t pn532_send_data(uint8_t *data,uint16_t len);
    unp_state_t pn532_recv_data(uint8_t *data,uint16_t len);
    bool is_pn532_enable();

    void deal_with_send_ack(uint8_t * data,uint16_t len);
    void deal_with_pn532_check(void);
    void deal_with_pn532_wakeup(void);
    void deal_with_pn532_setfree(void);
};
