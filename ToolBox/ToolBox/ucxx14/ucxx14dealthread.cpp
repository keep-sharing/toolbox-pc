#include "ucxx14dealthread.h"
#include <QDateTime>
#include <QTimer>
#include <QTimeZone>

UCXX14DealThread::UCXX14DealThread(QObject *parent) : QObject(parent)
{
    m_loginPwd = "";
}

UCXX14DealThread::~UCXX14DealThread()
{
    delete m_qunp;
}

void UCXX14DealThread::run()
{
    m_qunp = new QunpUCXX14();
    qDebug()<<"Thread id:"<<QThread::currentThreadId();
    connect(m_qunp,SIGNAL(getImageSize(uint32_t)),this,SLOT(imageProgress(uint32_t)),Qt::UniqueConnection);
}

void UCXX14DealThread::imageProgress(uint32_t pktsize)
{
...
void UCXX14DealThread::onConnectingDevice(SerialInfo s)
{
...
void UCXX14DealThread::onGetProductName(QString proName,QString swver, QString hwver, bool isNFC)
{
    m_proName = proName;
    m_swver = swver;
    m_hwver = hwver;
...
bool UCXX14DealThread::openSerialPort()
{
...
void UCXX14DealThread::closeSerialPort()
{
    m_qunp->close_serialport();
}

void UCXX14DealThread::onRequestToDevice(int type, QVariantList list)
{
    ...
    m_qunp->set_comnct_type(m_bNFC ? QunpUCXX14::COMNCT_NFC : QunpUCXX14::COMNCT_UART);
...
/**
 * 读状态页
 * @brief UCXX14DealThread::getStatusPage
 * @param list
 */
void UCXX14DealThread::getStatusPage(QVariantList list)
{
    Q_UNUSED(list)
    ...
    QunpUCXX14::unp_mobile_state_t state;
...
/**
 * 读General
 * @brief UCXX14DealThread::getGeneralPage
 * @param list
 */
void UCXX14DealThread::getGeneralPage(QVariantList list)
{
    Q_UNUSED(list)
...
//    curDevApplicationmode = am;
    ...
    if(structSupportModel.data){
        QunpUCXX14::app_tcpudp_t tcp_read;
        ...
        QunpUCXX14::app_mqtt_t mqtt_read;
        ...
        QunpUCXX14::unxx_onenet_setting_t onenet_read;
    ...
    if(structSupportModel.nb){
        QunpUCXX14::unxx_ali_setting_t ali;
    ...
    //onenet
    QunpUCXX14::unxx_onenet_setting_t onenet;
    ...
    QunpUCXX14::cellular_network_t cn;
...
void UCXX14DealThread::getGeneralGPIO(QVariantList list)
{
...
/**
 * 设置Genneral
 * @brief UCXX14DealThread::setGeneralPage
 * @param list
 */
void UCXX14DealThread::setGeneralPage(QVariantList list)
{
    ...
    ret = m_qunp->unp_set_cellular_net_type(static_cast<QunpUCXX14::cellular_network_t>(map.value("networkType").toInt()));

    if(structSupportModel.cloud){
        ret = m_qunp->unp_set_cloud_cycle(static_cast<uint32_t>(map.value("reportingInterval").toInt()));

        if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_AWS
    #ifdef ODM_LHR_AZURE
            || static_cast<app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_AZURE
    #endif
                ){
        ...
        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_TCP){
            QunpUCXX14::app_tcpudp_t tcp_set;
        ...
        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_UDP){
            QunpUCXX14::app_tcpudp_t udp_set;
        ...
        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_MQTT){
            QunpUCXX14::app_mqtt_t mqtt_set;
        ...
        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_URSALINK_CLOUD){
            ret = m_qunp->unp_set_mqtt_keepalive(static_cast<uint32_t>(map.value("keepAliveInterval").toInt()));

        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_ALI){
            QunpUCXX14::unxx_ali_setting_t ali;
            ...
            ali.qos = static_cast<QunpUCXX14::Qos>(map.value("aliyunQoS").toInt());
            m_qunp->unp_set_ali_cfg(&ali);
        }else if(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()) == QunpUCXX14::APP_ONENET){
            QunpUCXX14::unxx_onenet_setting_t onenet;
            onenet.qos=0;
    ...
    if(structSupportModel.cloud){
        if(m_structPageGeneral.application_mode != map.value("applicationMode").toInt()){
            ret = m_qunp->unp_set_app_mode(static_cast<QunpUCXX14::app_mode_t>(map.value("applicationMode").toInt()));
            waitTime(8000);
        }
    #ifdef ODM_LHR_AZURE
...
/**
 * 导入认证文件
 * @brief UCXX14DealThread::setImportAwdFile
 * @param list
 */
void UCXX14DealThread::setImportAwdFile(QVariantList list)
{
 ...
 * @param list
 */
void UCXX14DealThread::getLorawanPage(QVariantList list)
{
    Q_UNUSED(list)
 ...
 * @param list
 */
void UCXX14DealThread::setLorawanPage(QVariantList list)
{
    QVariantMap map = list.at(0).toMap();
    QString otaTmp;
...
/**
 * 读设置
 * @brief UCXX14DealThread::getSettingPage
 * @param list
 */
void UCXX14DealThread::getSettingPage(QVariantList list)
{
    Q_UNUSED(list)
...
/**
 * 写入设置界面
 * @brief UCXX14DealThread::setSettingPage
 * @param list
 */
void UCXX14DealThread::setSettingPage(QVariantList list)
{
...
/**
 * 升级
 * @brief UCXX14DealThread::toUpgrade
 * @param list
 */
void UCXX14DealThread::toUpgrade(QVariantList list)
{
    QVariantMap map = list.at(0).toMap();
    uint8_t *pdata;
 ...
 * @param showTip
 */
void UCXX14DealThread::resetDevice(QVariantList list, bool showTip)
{
    Q_UNUSED(showTip)
    Q_UNUSED(list)
...
bool UCXX14DealThread::strIsIp(QString str)
{
    if(str.split(".").size() == 4){
...
void UCXX14DealThread::setSupportModel(QString customBin)
{
    bool ok=0;
 ...
 * @return
 */
int UCXX14DealThread::setLoraChannel(QVariantList list)
{
...
int UCXX14DealThread::setLoraChannel2(QVariantList list)
{
    QVariantMap map = list.at(0).toMap().value("loraChannel").toMap();
...
void UCXX14DealThread::waitTime(int mesc)
{
...
QString UCXX14DealThread::hexToString(uint8_t *in, int len)
{
    QString tmp,a;
...
void UCXX14DealThread::stringToHex(QString dataStr, uint8_t *out)
{
    uint8_t dataHex[256] = {0};
    size_t offset=0;
    bool ok=0;
...
int UCXX14DealThread::stringToHex2(QString dataStr, uint8_t *out)
{
    uint8_t dataHex[256] = {0};
    int offset=0;
    bool ok=0;
