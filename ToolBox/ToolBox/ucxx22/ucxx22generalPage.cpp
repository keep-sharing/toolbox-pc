...
void UCXX22Model::onRecvGeneral(UCXX22DealThread::StructPageGeneral info)
{
    if(info.readSpecial == 9999){
        qDebug()<<info.readSpecialType<<info.counterNum<<info.gpioCounter;
        switch (info.readSpecialType) {
        case UCXX22DealThread::RNS_GPIO2_COUNTER_START:{
            ...
            break;
        case UCXX22DealThread::RNS_GPIO2_COUNTER_REFRESH:{
            if(info.counterNum == 1){
...
//                n1GpioCounterAnimation->start();
            ...
            break;
        case UCXX22DealThread::RNS_GPIO2_COUNTER_CLEAR:{
            if(info.counterNum == 1){
                if(info.gpioCounterStart.toInt() == 1){
                    ui->gpioCounterStart_btn->setText(tr("Stop"));
                }else{
    ...
    if(m_dealThread->structSupportModel.cloud){
        ...
        if(info.application_mode == QunpUCXX22::APP_NONE){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_NONE));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_NONE);

        }else if(info.application_mode == QunpUCXX22::APP_URSALINK_CLOUD){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_URSALINK_CLOUD));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_URSALINK_CLOUD);
        ...
        }else if(info.application_mode == QunpUCXX22::APP_AWS){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_AWS));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_AWS);
            ...
            ui->mqttTlsCliCerFile_frame->setEnabled(false);
            ui->mqttTlsCliKeyFile_frame->setEnabled(false);
        }else if(info.application_mode == QunpUCXX22::APP_TCP){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_TCP));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_TCP);
            ...
            ui->mqttTlsCliCerFile_frame->setEnabled(false);
            ui->mqttTlsCliKeyFile_frame->setEnabled(false);
        }else if(info.application_mode == QunpUCXX22::APP_UDP){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_UDP));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_UDP);
            ...
            ui->mqttTlsCliCerFile_frame->setEnabled(false);
            ui->mqttTlsCliKeyFile_frame->setEnabled(false);
        }else if(info.application_mode == QunpUCXX22::APP_MQTT){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_MQTT));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_MQTT);
        ...
        }else if(info.application_mode == QunpUCXX22::APP_ALI){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_ALI));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_ALI);
            ...
            ui->mqttTlsCliCerFile_frame->setEnabled(false);
            ui->mqttTlsCliKeyFile_frame->setEnabled(false);
        }else if(info.application_mode == QunpUCXX22::APP_ONENET){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_ONENET));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_ONENET);
    ...
    #ifdef ODM_LHR_AZURE
        else if(info.application_mode == QunpUCXX22::APP_AZURE){
            ui->cloudApplicationMode_cbx->setCurrentIndex(ui->cloudApplicationMode_cbx->findData(QunpUCXX22::APP_AZURE));
            on_cloudApplicationMode_cbx_currentIndexChanged(QunpUCXX22::APP_AZURE);
    ...
    ui->osh_lineEdit->setText(info.osh1);
    ui->osl_lineEdit->setText(info.osl1);

    ui->osh2_lineEdit->setText(info.osh2);
    ui->osl2_lineEdit->setText(info.osl2);

    if(ui->unit_lineEdit->isVisible()){
        QSettings settings(m_iniPath,QSettings::IniFormat);
        settings.beginGroup("AiUnit");
        ui->unit_lineEdit->setText( settings.value("A"+m_statusPageInfo.sn).toString().isEmpty() ? "mA" : settings.value("A"+m_statusPageInfo.sn).toString());
        ui->unit2_lineEdit->setText(settings.value("B"+m_statusPageInfo.sn).toString().isEmpty() ? "mA" : settings.value("B"+m_statusPageInfo.sn).toString());
        settings.endGroup();
    }
    emit waitingVisible(false);
}
...
/**
 * 修改密码
 * @brief UCXX22Model::on_changePwd_cb_clicked
 * @param checked
 */
void UCXX22Model::on_changePwd_cb_clicked(bool checked)
{
...
/**
 * 上云版本模式变化
 * @brief UCXX22Model::on_cloudApplicationMode_cbx_currentIndexChanged
 * @param index
 */
void UCXX22Model::on_cloudApplicationMode_cbx_currentIndexChanged(int index)
{
    ...
    if(index == QunpUCXX22::APP_NONE){
    }else if(index == QunpUCXX22::APP_URSALINK_CLOUD){
        ui->generalKeepAliveInterval_frame->setVisible(true);

    }else if(index == QunpUCXX22::APP_AWS ){
        ui->generalKeepAliveInterval_frame->setVisible(true);
        ui->generalReportingIntl_frame->setVisible(true);
...
//        ui->dataPollInterval_frame->setVisible(true);
    ...
    }else if(index == QunpUCXX22::APP_TCP){
...
//        ui->dataPollInterval_frame->setVisible(true);
    ...
    }else if(index == QunpUCXX22::APP_UDP){
        ui->customHeartbeatCb_frame->setVisible(true);
        ui->tcpDestIpAddr_frame->setVisible(true);
        ui->generalReportingIntl_frame->setVisible(true);
//        ui->dataPollInterval_frame->setVisible(true);
...
/**
 * 自定义心跳包勾选框
 * @brief UCXX22Model::on_customHeartbeat_cb_clicked
 * @param checked
 */
void UCXX22Model::on_customHeartbeat_cb_clicked(bool checked)
{
...
void UCXX22Model::on_requireResponse_cb_clicked(bool checked)
{
    ui->requireResponse_cb->setChecked(checked);
    ui->responseContent_frame->setVisible(checked);

    if(!checked){
        if(ui->cloudApplicationMode_cbx->currentData().toInt() == QunpUCXX22::APP_UDP){
...
void UCXX22Model::on_mqttUserCreEnable_cb_clicked(bool checked)
{
    ui->mattUserCreUsername_frame->setEnabled(checked);
    ui->mattUserCrePwd_frame->setEnabled(checked);
}

void UCXX22Model::on_mqttTlsEnable_cb_clicked(bool checked)
{
    if(m_generalPageInfo.application_mode == QunpUCXX22::APP_MQTT){
...
/**
 * CA File 游览
 * @brief UCXX22Model::on_awsCASelectFile_btn_clicked
 */
void UCXX22Model::on_awsCASelectFile_btn_clicked()
{
...
/**
 * CA 导入
 * @brief UCXX22Model::on_awsCAImportFile_btn_clicked
 */
void UCXX22Model::on_awsCAImportFile_btn_clicked()
{
    QVariantMap map;
    ...
    if(m_bcaFileBrowse)
        emit requestToDevice(UCXX22DealThread::CONFIG_IMPORT_AWS_FILE,list);

    m_bcaFileBrowse = false;
}
/**
 * CA 删除
 * @brief UCXX22Model::on_awsCADelFile_btn_clicked
 */
void UCXX22Model::on_awsCADelFile_btn_clicked()
{
    QVariantMap map;
    map.insert("fileType","CA");
...
/**
 * 设备证书选择
 * @brief UCXX22Model::on_awsCliCerSelectFile_btn_clicked
 */
void UCXX22Model::on_awsCliCerSelectFile_btn_clicked()
{
#ifdef ODM_LHR_AZURE
...
/**
 * 设备证书导入
 * @brief UCXX22Model::on_awsCliCerImportFile_btn_clicked
 */
void UCXX22Model::on_awsCliCerImportFile_btn_clicked()
{
    QVariantMap map;
...
/**
 * 设备证书删除
 * @brief UCXX22Model::on_awsCliCerDelFile_btn_clicked
 */
void UCXX22Model::on_awsCliCerDelFile_btn_clicked()
{
    QVariantMap map;
    map.insert("fileType","Client Certificate");
    map.insert("delete",true);
...
/**
 * 私钥选择
 * @brief UCXX22Model::on_awsCliKeySelectFile_btn_clicked
 */
void UCXX22Model::on_awsCliKeySelectFile_btn_clicked()
{
#ifdef ODM_LHR_AZURE
...
/**
 * 私钥导入
 * @brief UCXX22Model::on_awsCliKeyImportFile_btn_clicked
 */
void UCXX22Model::on_awsCliKeyImportFile_btn_clicked()
{
    QVariantMap map;
...
/**
 * 私钥删除
 * @brief UCXX22Model::on_awsCliKeyDelFile_btn_clicked
 */
void UCXX22Model::on_awsCliKeyDelFile_btn_clicked()
{
    QVariantMap map;
    map.insert("fileType","Client Key");
    map.insert("delete",true);
...
/**
 * ODM Azure的模式下 选择
 * @brief UCXX22Model::on_azureMode_cbx_currentIndexChanged
 * @param index
 */
void UCXX22Model::on_azureMode_cbx_currentIndexChanged(int index)
{
    Q_UNUSED(index)
#ifdef ODM_LHR_AZURE
...
/**
 * 保存Genenal页面信息
 * @brief UCXX22Model::on_generalSave_btn_clicked
 */
void UCXX22Model::on_generalSave_btn_clicked()
{
    ...
    if(ui->cloudApplicationMode_cbx->currentData().toInt() == QunpUCXX22::APP_ALI){
    ...
    if(!ui->osh_lineEdit->text().isEmpty()){
        if(!ui->osh_lineEdit->text().remove("0").remove("1").remove("2").remove("3").remove("4").remove("5").remove("6").remove("7").remove("8").remove("9").remove("-").remove(".").isEmpty()){
            ui->osh_lineEdit->setStyleSheet(ERROR_INPUT);
            showWarning(this,tr("Attention"),tr("Only numbers and decimal point are allowed."));
            return;
        }
    }
    if(!ui->osl_lineEdit->text().isEmpty()){
        if(!ui->osl_lineEdit->text().remove("0").remove("1").remove("2").remove("3").remove("4").remove("5").remove("6").remove("7").remove("8").remove("9").remove("-").remove(".").isEmpty()){
            ui->osl_lineEdit->setStyleSheet(ERROR_INPUT);
            showWarning(this,tr("Attention"),tr("Only numbers and decimal point are allowed."));
            return;
        }
    }
    if(ui->osh_lineEdit->text().contains("-") && ui->osh_lineEdit->text().at(0) != '-'){
        ui->osh_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));
        return;
    }
    if(ui->osl_lineEdit->text().contains("-") && ui->osl_lineEdit->text().at(0) != '-'){
        ui->osl_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));
        return;
    }
    if(ui->osh_lineEdit->text().trimmed().toDouble() >65535 || ui->osh_lineEdit->text().trimmed().toDouble()<-65535){
        ui->osh_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));
        return;
    }
    if(ui->osl_lineEdit->text().trimmed().toDouble() >65535 || ui->osl_lineEdit->text().trimmed().toDouble()<-65535){
        ui->osl_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));
        return;
    }

    if((ui->osh_lineEdit->text().toDouble() <= ui->osl_lineEdit->text().toDouble()) &&
            (!ui->osh_lineEdit->text().isEmpty()||!ui->osl_lineEdit->text().isEmpty()) &&
            (ui->osh_lineEdit->text().toDouble()!=0.00 || ui->osl_lineEdit->text().toDouble()!=0.00)){
        ui->osl_lineEdit->setStyleSheet(ERROR_INPUT);
        ui->osh_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("The value of Osh must be bigger than the value of Osl."));
        return;
    }
    ...
    if(!ui->osl2_lineEdit->text().isEmpty()){
        if(!ui->osl2_lineEdit->text().remove("0").remove("1").remove("2").remove("3").remove("4").remove("5").remove("6").remove("7").remove("8").remove("9").remove("-").remove(".").isEmpty()){
            ui->osl2_lineEdit->setStyleSheet(ERROR_INPUT);
            showWarning(this,tr("Attention"),tr("Only numbers and decimal point are allowed."));

            return;
        }
    }
    if(ui->osh2_lineEdit->text().contains("-") && ui->osh2_lineEdit->text().at(0) != '-'){
        ui->osh2_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));

        return;
    }
    if(ui->osl2_lineEdit->text().contains("-") && ui->osl2_lineEdit->text().at(0) != '-'){
        ui->osl2_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));

        return;
    }
    if(ui->osh2_lineEdit->text().trimmed().toDouble() >65535 || ui->osh2_lineEdit->text().trimmed().toDouble()<-65535){
        ui->osh2_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));

        return;
    }
    if(ui->osl2_lineEdit->text().trimmed().toDouble() >65535 || ui->osl2_lineEdit->text().trimmed().toDouble()<-65535){
        ui->osl2_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("Valid Range：-65535-65535."));

        return;
    }
    if((ui->osh2_lineEdit->text().toDouble() <= ui->osl2_lineEdit->text().toDouble()) &&
            (!ui->osh2_lineEdit->text().isEmpty()||!ui->osl2_lineEdit->text().isEmpty()) &&
            (ui->osh2_lineEdit->text().toDouble()!=0.00 || ui->osl2_lineEdit->text().toDouble()!=0.00)){
        ui->osl2_lineEdit->setStyleSheet(ERROR_INPUT);
        ui->osh2_lineEdit->setStyleSheet(ERROR_INPUT);
        showWarning(this,tr("Attention"),tr("The value of Osh must be bigger than the value of Osl."));
        return;
    ...
    if(!ui->osh_lineEdit->text().isEmpty() ){
        if( (ui->osh_lineEdit->text().toDouble()!=0.00 && ui->osl_lineEdit->text().toDouble()!=0.00)
                || (ui->osh_lineEdit->text().toInt()!=0.00 && ui->osl_lineEdit->text().toInt()!=0.00) ){
            if(ui->unit_lineEdit->text().isEmpty()){
                ui->unit_lineEdit->setStyleSheet(ERROR_INPUT);
                showWarning(this,tr("Attention"),tr("This field is required."));
    ...
    if(!ui->osh2_lineEdit->text().isEmpty() ){
        if( (ui->osh2_lineEdit->text().toDouble()!=0.00 && ui->osl2_lineEdit->text().toDouble()!=0.00)
                || (ui->osh2_lineEdit->text().toInt()!=0 && ui->osl2_lineEdit->text().toInt()!=0)){
            if(ui->unit2_lineEdit->text().isEmpty()){
                ui->unit2_lineEdit->setStyleSheet(ERROR_INPUT);
                showWarning(this,tr("Attention"),tr("This field is required."));
    ...
    map.insert("osl",(ui->osl_lineEdit->text().toDouble()==0.0 && ui->osh_lineEdit->text().toDouble()==0.0) ? "4":ui->osl_lineEdit->text().trimmed());
    map.insert("osh",(ui->osl_lineEdit->text().toDouble()==0.0 && ui->osh_lineEdit->text().toDouble()==0.0) ?"20":ui->osh_lineEdit->text().trimmed());
    map.insert("osl2",(ui->osl2_lineEdit->text().toDouble()==0.0 && ui->osh2_lineEdit->text().toDouble()==0.0) ?"4":ui->osl2_lineEdit->text().trimmed());
    map.insert("osh2",(ui->osl2_lineEdit->text().toDouble()==0.0 && ui->osh2_lineEdit->text().toDouble()==0.0) ?"20":ui->osh2_lineEdit->text().trimmed());

    m_generalPageInfo.osl1 = map.value("osl").toString();
    m_generalPageInfo.osh1 = map.value("osh").toString();

    m_generalPageInfo.osl2 = map.value("osl2").toString();
    m_generalPageInfo.osh2 = map.value("osh2").toString();

    QSettings settings(m_iniPath,QSettings::IniFormat);
    settings.beginGroup("AiUnit");
    if(!ui->unit_lineEdit->text().isEmpty()){
        settings.setValue("A"+m_statusPageInfo.sn,ui->unit_lineEdit->text().trimmed());
    }
    if(!ui->unit2_lineEdit->text().isEmpty()){
        settings.setValue("B"+m_statusPageInfo.sn,ui->unit2_lineEdit->text().trimmed());
    }
    settings.endGroup();
    ...
    emit requestToDevice(UCXX22DealThread::CONFIG_WRITE_GENERAL,list);

    if(ui->cloudApplicationMode_cbx->currentData().toInt() == QunpUCXX22::APP_AWS
#ifdef ODM_LHR_AZURE
       || ui->cloudApplicationMode_cbx->currentData().toInt() == QunpUCXX22::APP_AZURE
#endif
            ){
        ui->awsCAFile_frame->setEnabled(true);
        ui->awsCliCerFile_frame->setEnabled(true);
        ...
        ui->mqttTlsCliCerFile_frame->setEnabled(false);
        ui->mqttTlsCliKeyFile_frame->setEnabled(false);
    }else if(ui->cloudApplicationMode_cbx->currentData().toInt() == QunpUCXX22::APP_MQTT){
        ui->awsCAFile_frame->setEnabled(false);
        ui->awsCliCerFile_frame->setEnabled(false);
        ui->awsCliKeyFile_frame->setEnabled(false);
 ...
 * @brief UCXX22Model::on_gpioType_cbx_currentIndexChanged
 * @param index
 */
void UCXX22Model::on_gpioType_cbx_currentIndexChanged(int index)
{
...
/**
 * 1114 1122 开始计数
 * @brief UCXX22Model::on_gpioCounterStart_btn_clicked
 */
void UCXX22Model::on_gpioCounterStart_btn_clicked()
{
    QVariantMap map;
    map.insert("readType",UCXX22DealThread::RNS_GPIO2_COUNTER_START);
    ...
    emit requestToDevice(UCXX22DealThread::CONFIG_READ_GENERAL_SPECIAL,list);
}

/**
 * 1114 1122 刷新
 * @brief UCXX22Model::on_gpioCounterRefresh_btn_clicked
 */
void UCXX22Model::on_gpioCounterRefresh_btn_clicked()
{
    QVariantMap map;
    map.insert("readType",UCXX22DealThread::RNS_GPIO2_COUNTER_REFRESH);
    map.insert("counterNum",1);
    QVariantList list;
    list.append(map);
...
/**
 * 1114 1122 清空
 * @brief UCXX22Model::on_gpioCounterClear_btn_clicked
 */
void UCXX22Model::on_gpioCounterClear_btn_clicked()
{
    QVariantMap map;
    map.insert("readType",UCXX22DealThread::RNS_GPIO2_COUNTER_CLEAR);
    map.insert("counterNum",1);
    QVariantList list;
    list.append(map);
